---
title: "Code for Geometric Integrated Distribution Models"
output: pdf_document
date: "2025-09-05"
---
...
### Set directory 
dir <- "/Users/justinvanee/Library/Mobile Documents/com~apple~CloudDocs/Documents/brook_trout_postdoc/gIDM"
...
count_data <- 
  read.csv(file= file.path(dir, "Data", "NC_count.csv")) %>%
  # Define new columns 
  mutate(
    Date=mdy(Date),
    year=year(Date),
...
# Get global min_distance threshold
min_distance <- min(min_distance_per_year$min_distance, na.rm = TRUE)-1

### Extract unique site geometries (across all years)
unique_sites <- count_data %>%
  distinct(Site_ID, .keep_all = TRUE)

### Create a spatial graph of proximity
within_dist_list <- st_is_within_distance(unique_sites, dist = min_distance)

### Build a graph where nodes are sites and edges connect sites within min_distance
g <- graph_from_adj_list(within_dist_list, mode = "all")

### Identify connected components
clusters <- components(g)

### Assign new site IDs based on cluster membership
unique_sites <- unique_sites %>%
  mutate(sid = clusters$membership)

### Join new_sid back to original count_data
...
all_occupancy <- read.csv(file = file.path(dir, "Data", "occupancy_nc.csv")) %>%
  filter(
...
edges_raw <-
  st_read(file.path(dir, "Data", "lsn4.ssn", "edges.shp")) %>%
...
### Transform to WGS84 (Daymet requires lat/lon)
...
### Function to extract Daymet data for a site
...
### Save data
#save(temp_data, file=file.path(dir, "Output", "Daymet_raw.RData"))

### Summarize seasonal max temps
...
### Pivot wider: one row per site/year, with summer and winter temps
Daymet <- seasonal_max %>%
...
### Spring 2024 not released yet. Set to previous years values 
Daymet <- 
  Daymet %>%
  group_by(sid) %>%
  mutate(across(
...
### Save data
save(Daymet, file=file.path(dir, "Outputs", "Daymet.RData")) 
...
### Spatial plot with facets by year
ggplot(daymet_long) +
...
watersheds_TN <- 
  st_read(paste0(file.path(dir, "Data", "watershed_boundaries_TN", "WBDHU"), resolution, ".shp")) %>%
  st_transform(edges, crs = 32119)

watersheds_SE <- 
  st_read(file.path(dir, "Data", "watershed_boundaries_SE",
                    paste0("WBDHU", resolution, ".shp"))) %>%
  st_transform(edges, crs = 32119)

watersheds_ME <- 
  st_read(file.path(dir, "Data", "watershed_boundaries_ME",
                    paste0("WBDHU", resolution, ".shp"))) %>%
  st_transform(edges, crs = 32119)
...
ggsave(plot=single_network_inset_nc, 
       filename=file.path(dir, "Figures", "single_network_inset_nc_206.png"),
...
### load in Daymet data
load(file=file.path(dir, "Outputs", "Daymet.RData"))
...
###
### Flatten and consolidate objects common to both data types
###


### Filter for sites with sample point 
obs_idx  <- sort(base::union(z_site_idx, y_site_idx))

### Extract observed pids 
pids_obs <- pids[obs_idx]

### Parse netID and pid from pids_obs names
obs_map <- tibble(
  full_name = names(pids_obs),
  pid       = as.character(pids_obs)
) %>%
  mutate(
    # Extract netID by matching against known network names in D
    netID = stringr::str_remove(stringr::str_extract(full_name, paste(names(D), collapse = "|")), "^dist\\.net")
  )

### Create full block diagonal matrix 
D_big <- bdiag(D)

### Subset for observed sites 
D_obs <- as.matrix(D_big[obs_idx, obs_idx])

### Set off diagonal of blocks to Inf 
D_obs[D_obs==0] <- Inf
diag(D_obs)     <- 0 

### Set names 
rownames(D_obs) <- obs_map$pid
colnames(D_obs) <- obs_map$netID

### Create list for splitting matrices 
netID_split <- split(seq_along(obs_map$netID), obs_map$netID)

# Build a list of matrices, one per netID
D_obs_list <- lapply(netID_split, function(idx) {
  D_obs[idx, idx, drop = FALSE]   # select rows and columns for this netID
})
...
save(data, file=file.path(dir, "Outputs", "data.RData"))
...
walk(fit_list[c(1,3)], ~ hist(.x$N_acc / ncol(.x$N),
...
### Hyperparameters
n_keep <- 1000
thin_seq <- seq(from = 1, to = n_mcmc, length.out = n_keep)
burnin <- 10
...
### Plot 
walk(traceplots, plot)

```

## Fit Poisson & negative binomial IDMs (stan)
...
###
### Fit Poisson IDM (HMC, estimated run time over ~1 week!)
###

trout_pHMC <- mod$sample(
  data = stan_data,
  chains = 3,
  parallel_chains = 3,
  iter_warmup = 200,
...
### summary
trout_pHMC$summary(variables = c("sigma2_eta", "sigma2_zeta", "sigma2_xi", "rho_xi", "nu",
                                  "gamma", "beta", "xi", "zeta"))
 
# extract draws
draws_pHMC <- as_draws_array(trout_pHMC$draws(
  variables = c("sigma2_eta", "sigma2_zeta", "sigma2_xi", 
                "rho_xi", "nu",
                "gamma", "beta", "xi", "zeta",
                "log_prob", "log_lambda")
))
...
# Output 
save(trout_pHMC, file=file.path(dir, "Outputs", "trout_pHMC.RData"))

###
### Traceplots
###

pars <- c(paste0("sigma2_eta[", 1:data$Q, "]"), "sigma2_zeta", "sigma2_xi", "rho_xi",
          paste0("gamma[", 1:data$p_H, "]"), paste0("beta[", 1:data$p_X, "]"))
...
# Output 
save(trout_pVI, file=file.path(dir, "Outputs", "trout_pVI.RData"))

# Compile the negative binomial IDM
mod <- cmdstan_model(file.path(dir, "algorithms", "IDM_NB.stan"))
...
### summary
trout_nbHMC$summary(variables = c("sigma2_eta", "sigma2_zeta", "sigma2_xi", "rho_xi", "nu",
                                  "gamma", "beta", "xi", "zeta"))
 
# extract draws
draws_nbHMC <- as_draws_array(trout_nbHMC$draws(
  variables = c("sigma2_eta", "sigma2_zeta", "sigma2_xi", 
                "rho_xi", "nu",
                "gamma", "beta", "xi", "zeta",
                "log_prob", "log_lambda")
...
# Output 
save(trout_nbHMC, file=file.path(dir, "Outputs", "trout_nbHMC.RData"))

###
### Traceplots
###

pars <- c(paste0("sigma2_eta[", 1:data$Q, "]"), "sigma2_zeta", "sigma2_xi", "rho_xi", "nu",
          paste0("gamma[", 1:data$p_H, "]"), paste0("beta[", 1:data$p_X, "]"))
...
load(file=file.path(dir, "Outputs", "fit_list.RData"))
load(file=file.path(dir, "Outputs", "data.RData"))
load(file=file.path(dir, "Outputs", "trout_pVI.RData"))
load(file=file.path(dir, "Outputs", "trout_pHMC.RData"))
load(file=file.path(dir, "Outputs", "trout_nbHMC.RData"))

### Extract model fit 
joint=fit_list$joint
...
z <- data$z_flat
n_y <- data$n_y
### We treat posterior median abundance from geometric IDM as truth
### This is reasonable because small variation in inferred site-level abundance 
### That is primarily a function of depeletion detection model which is identical across IDMs
y <- apply(joint$N, 1, median)

###
### PPD (Geometric IDM with PG augmentation)
###

burnin <- 1000
samples <- 1000
thin_seq <- seq(from=burnin, to=ncol(joint$log_lambda[-c(1:n_z),]), length.out=samples)

### Extract probability of presence for presence/absence sites
G_psi_z <- logit_inv(joint$log_lambda[1:n_z,thin_seq])

### Extract expected abundance for depletion sites
G_lambda_y <- exp(joint$log_lambda[-(1:n_z),thin_seq])

### Calculate standardization for scaled RPS 
G_N <-
  rnbinom(
    samples * n_y, 
    size = 1, 
...
G_standardize_y <- sapply(1:n_y, function(i) mean(abs(G_N[i, ] - sample(G_N[i, ]))))
  
  

###
### PPD (Poisson IDM with HMC)
###

### Extract probability of occupancy for presence/absence sites
pHMC_psi_z <- 
  trout_pHMC$draws("log_prob") %>% 
  subset_draws(chain=1) %>% # just use first chain 
  as_draws_matrix() %>%
  t() %>%
  cloglog_inv()
...
pHMC_N <-
  rpois(
    samples * n_y, 
    lambda = pHMC_lambda_y
  ) %>%
  matrix(
    n_y,
    samples
  )
pHMC_standardize_y <- sapply(1:n_y, function(i) mean(abs(pHMC_N[i, ] - sample(pHMC_N[i, ]))))

### Extract effective sample size for beta
pHMC_beta_ess <- 
  trout_pHMC$draws("beta") %>% 
  apply(3, mcmcse::ess) %>% # average across chains 
  apply(2, mean)

###
### PPD (negative binomial IDM with HMC)
###

### Extract probability of occupancy for presence/absence sites
mu_z <- 
  trout_nbHMC$draws("log_prob") %>% 
  subset_draws(chain=1) %>% # just use first chain 
  as_draws_matrix() %>%
  t() %>%
  exp()
nu <- trout_nbHMC$draws("nu") %>% 
  subset_draws(chain=1) %>%
  as.vector()
nbHMC_psi_z <- t(sapply(1:n_z, function(i) 1 - (nu / (nu + mu_z[i,]))^nu))
...
nbHMC_standardize_y <- sapply(1:n_y, function(i) mean(abs(nbHMC_N[i, ] - sample(nbHMC_N[i, ]))))

### Extract effective sample size for beta
nbHMC_beta_ess <- 
  trout_nbHMC$draws("beta") %>% 
  apply(3, mcmcse::ess) %>% # average across chains 
  apply(2, mean)

###
### PPD (Poisson IDM with VI)
###

### Extract probability of occupancy for presence/absence sites
pVI_psi_z <- 
  trout_pVI$draws("log_prob") %>% 
  subset_draws(chain=1) %>% # just use first chain 
  as_draws_matrix() %>%
  t() %>%
  cloglog_inv()
...
pVI_standardize_y <- sapply(1:n_y, function(i) mean(abs(pVI_N[i, ] - sample(pVI_N[i, ]))))

###
### Ranked Probability Score (presence/absence, simplifies to RMSE)
###

## geometric IDM implement with PG augmentation 
G_RPS_z    <- apply(G_psi_z, 2, function(x) (z-x)^2)
G_site_z   <- apply(G_RPS_z, 1, mean)
# Note this equals crps_nbinom(y[i], size = 1, prob = psi_y[i,]) where psi_y=logit_inv(log_lambda)
G_RPS_y    <- sapply(1:n_y, function(i) crps_nbinom(y[i], size = 1, mu = G_lambda_y[i, ]))
G_site_y   <- apply(G_RPS_y, 2, mean)
# Calculate scaled RPS, equation (3) in Bolin & Wallin 2023 
G_scaled_y <- 0.5 * ( 1 + G_site_y / G_standardize_y + log(2 * G_standardize_y) )

## Poisson IDM (HMC) 
pHMC_RPS_z     <- apply(pHMC_psi_z, 2, function(x) (z-x)^2)
pHMC_site_z    <- apply(pHMC_RPS_z, 1, mean)
pHMC_RPS_y     <- sapply(1:n_y, function(i) crps_pois(y[i], lambda = pHMC_lambda_y[i, ]))
pHMC_site_y    <- apply(pHMC_RPS_y, 2, mean)
pHMC_scaled_y  <- 0.5 * ( 1 + pHMC_site_y / pHMC_standardize_y + log(2 * pHMC_standardize_y) )


## negative binomial IDM (HMC) 
nbHMC_RPS_z     <- apply(nbHMC_psi_z, 2, function(x) (z-x)^2)
nbHMC_site_z    <- apply(nbHMC_RPS_z, 1, mean)
nbHMC_RPS_y     <- sapply(1:n_y, function(i) crps_nbinom(y[i], size = nu, mu = nbHMC_lambda_y[i, ]))
nbHMC_site_y    <- apply(nbHMC_RPS_y, 2, mean)
nbHMC_scaled_y  <- 0.5 * ( 1 + nbHMC_site_y / nbHMC_standardize_y + log(2 * nbHMC_standardize_y) )

## Poisson IDM (HMC) 
pVI_RPS_z    <- apply(pVI_psi_z, 2, function(x) (z-x)^2)
pVI_site_z   <- apply(pVI_RPS_z, 1, mean)
pVI_RPS_y    <- sapply(1:n_y, function(i) crps_pois(y[i], lambda = pVI_lambda_y[i, ]))
pVI_site_y   <- apply(pVI_RPS_y, 2, mean)
pVI_scaled_y <- 0.5 * ( 1 + pVI_site_y / pVI_standardize_y + log(2 * pVI_standardize_y) )
...
G     <- 1000 / (mean(apply(joint$beta[,-c(1:burnin)], 1, mcmcse::ess)) / fit_list$time[3]) 
pHMC  <- 1000 / (mean(pHMC_beta_ess) / (mean(trout_pHMC$time()$chains$total) / 60)) # Time in seconds 
nbHMC <- 1000 / (mean(nbHMC_beta_ess) / (mean(trout_nbHMC$time()$chains$total) / 60)) # Time in seconds 
pVI   <- as.vector(trout_pVI$time()$total) / 60  # Time in seconds 

# Function to summarize each matrix
mean_ci_format <- function(vector, digits = 2) {
  lwr <- quantile(vector, 0.05, na.rm = TRUE)
  upr <- quantile(vector, 0.95, na.rm = TRUE)
...
summary_df <- 
  # RPS presence/absence 
  rbind(
    pVI   = mean_ci_format(pVI_site_z),
    pHMC  = mean_ci_format(pHMC_site_z),
    nbHMC = mean_ci_format(nbHMC_site_z),
    G     = mean_ci_format(G_site_z)
  ) %>%
  cbind(
    # RPS abundance 
    rbind(
        pVI   = mean_ci_format(pVI_scaled_y),
        pHMC  = mean_ci_format(pHMC_scaled_y),
        nbHMC = mean_ci_format(nbHMC_scaled_y),
        G     = mean_ci_format(G_scaled_y)
      ),
    round(c(pVI, pHMC, nbHMC, G))
  )

# Assign column names
colnames(summary_df) <- c("Presence--Absence RPS",
                          "Count RPS",
                          "Time")

# Convert to xtable and print
latex_table <- xtable(summary_df, align = "lccc")
...
### Extract model fit 
joint <- fit_list$joint
n_mcmc <- ncol(joint$beta)
phi <- joint$phi[q_idx,]
sigma2_eta <- joint$sigma2_eta[q_idx,]
n_samples <- 100
...
for(l in 1:n_samples){
  ...
  # Effects per site
  eta_all_array[-pred_idx,l] <- joint$eta[eta_idx, thin_seq[l]]
  eta_all_array[pred_idx,l] <- 
  eta_pred <- 
    rmvn(1,
        mu=mean_mat %*% joint$eta[eta_idx, thin_seq[l]],
        sigma=var_mat)
  site_pred <- joint$zeta[zeta_idx, thin_seq[l]] + eta_pred
  
  for(t in 1:T){
    log_lambda_pred[t,,l] <- 
      X_pred[t,,] %*% joint$beta[,thin_seq[l]] +
      joint$xi[t,thin_seq[l]] +
      site_pred 
  }
...
save(intensity_pred, file=file.path(dir, "Outputs", "intensity_pred.RData"))
```
...
### load outputs & ssn
load(file=file.path(dir, "Outputs", "fit_list.RData"))
load(file=file.path(dir, "Outputs", "intensity_pred.RData"))
ssn.object <- ssn_import(path=file.path(dir, "Outputs", "nc.ssn"), overwrite = TRUE)
...
ggsave(plot = combined_plot,
       filename = file.path(dir, "Figures", "combined_predicted_density_netID_206.png"),
...
### Extract model fit 
joint=fit_list$joint
binary=fit_list$binary
count=fit_list$count
...
beta_joint_df <- as.data.frame(t(joint$beta)) %>%
...
# presence/absence
beta_presence_df <- as.data.frame(t(binary$beta)) %>%
  mutate(iter = row_number()) %>%
  pivot_longer(cols = starts_with("V"), names_to = "beta", values_to = "value") %>%
  mutate(beta = paste0("beta[", as.integer(str_remove(beta, "V")), "]"),
...
# count
beta_count_df <- as.data.frame(t(count$beta)) %>%
  mutate(iter = row_number()) %>%
  pivot_longer(cols = starts_with("V"), names_to = "beta", values_to = "value") %>%
  mutate(beta = paste0("beta[", as.integer(str_remove(beta, "V")), "]"),
        `Data Sources` = "count")
...
posterior_df <- 
  ...
  mutate(
    ...
    type=if_else(name %in% 
                   c("Precipitation Spring", "Precipitation Winter", 
                     "Temperature Summer", "Temperature Anomaly"),
                 "Temporal",
                 "Static"
    )
    )
...
### Save plot 
ggsave(beta_ridges, 
       file=file.path(dir, "Figures", "beta_ridges.png"), 
       width=8,
       height=4)
...
simulate_wrapper <- function(n) {
  simulate_data(n = n, n_ppd = n_ppd, z_prop = z_prop, p = p)
}
...
hist(map_dbl(data_list, ~mean(exp(.$log_lambda))))
...
metrics %>%
  group_by(Model) %>%
  summarise(across(time:ess_eta, mean, na.rm=TRUE)) 

# Look at differences between time per iteration and mixing (ESS) p(HMC) and g(PG) for n=500
metrics %>%
  filter(dataset >= 101 & dataset <= 200, # filters for n=500
         Model %in% c("p(HMC)", "g(PG)")) %>%
  select(dataset, Model, time, ess_beta) %>%
  pivot_wider(names_from = Model,
              values_from = c(time, ess_beta)) %>%
  mutate(
    time_ratio = `time_p(HMC)` / `time_g(PG)`,
    ess_ratio  = `ess_beta_p(HMC)` / `ess_beta_g(PG)`
  ) %>%
  summarise(
    mean_time_ratio = mean(time_ratio, na.rm = TRUE),
    mean_ess_ratio  = mean(ess_ratio, na.rm = TRUE)
  )

# Save output
save(metrics, file=file.path(dir, "Outputs", "metrics_p10.RData"))
```
...
load(file=file.path(dir, "Outputs", "metrics_p10.RData"))
...
metrics_long <- metrics %>%
  ...
  mutate(
    ...
    ess_eta_rate = log10(1000 / (ess_eta / time)),
    ess_N_rate = log10(1000 / (ess_N / time)),
    ### These are skewed so take log 
    bias_N = log(bias_N),
    sd_N = log(sd_N)
  ) %>%
  pivot_longer(
    cols = c(cov_beta, cov_eta, cov_N,
             bias_beta, bias_eta, bias_N,
             sd_beta, sd_eta, sd_N, 
             ess_beta_rate, ess_eta_rate, ess_N_rate),
    names_to = "metric",
    values_to = "value"
  ) %>%
  mutate(
    ...
    param = case_when(
      ...
      grepl("eta", metric) ~ "eta",
      grepl("N", metric) ~ "N"
    ),
  ) %>%
  mutate(Model = factor(Model, 
                        levels=c("p(HMC)", "p(VI)", "nb(HMC)", "g(PG)"),
                        labels=c("p(H)", "p(V)", "nb(H)", "g(P)")))
...
# Define the common ggplot layers and elements
common_plot_layers <- list(
  ...
  scale_fill_manual(values = wes_palette("AsteroidCity1", 4, type = "discrete")),
  labs(
    x = element_blank(),
    y = element_blank(),
    fill = element_blank()
...
beta_plot <- metrics_trimmed %>%
  ...
  common_plot_layers +
  labs(title = expression("A:" ~ bold(beta)))

### Boxplot of results (eta)
eta_plot <- metrics_trimmed %>%
  filter(param == "eta") %>%
  ggplot(aes(x = Model, y = value, fill = Model)) +
  common_plot_layers +
  labs(title = expression("B:" ~ bold(eta)))

### Boxplot of results (N)
N_plot <- metrics_trimmed %>%
  filter(param == "N") %>%
  ggplot(aes(x = Model, y = value, fill = Model)) +
  common_plot_layers +
  labs(title = expression("C:" ~ N))
...
ggsave(simulation_plot, 
       file=file.path(dir, "Figures", "simulation_plot_n.png"),
       scale=1.5,
       width=12,
       height=6)
...
simulate_wrapper <- function(nu) {
  simulate_data(n = n, n_ppd = n_ppd, z_prop = z_prop, p = p, nu = nu)
}
...
metrics %>%
  group_by(nu, Model) %>%
  summarise(across(c(time:nu_mean), 
                  .fns=~mean(.x, na.rm=TRUE))) 

# Save metrics 
save(metrics, file=file.path(dir, "Outputs", "metrics_p10_n100_nu.RData"))
...
load(file.path(dir, "Outputs", "metrics_p10_n100_nu.RData"))
...
### Remove outliers 
metrics_trimmed <- metrics_long %>%
  left_join(limits, by = c("metric_type", "nu", "param")) %>%
  filter(value >= lower, value <= upper) 
...
common_plot_layers <- list(
  ...
  scale_fill_manual(values = wes_palette("AsteroidCity1", 5, type = "discrete")),
  labs(
    x = element_blank(),
    y = element_blank(),
    fill = element_blank()
...
### Save plots 
ggsave(simulation_plot, 
       file=file.path(dir, "Figures", "simulation_plot_nu.png"),
       width=12,
       scale=1.5,
       height=6)
...
## Plot Poisson versus negative binomial PMFs 

```{r,}

# --- 1. Define Parameters and Count Range ---
lambda <- 10  # Mean (mu) for all distributions
N_max <- 30  # Max count value to plot
k <- 0:N_max # Sequence of count values

# Calculate Theoretical Probability Mass Function (PMF)
theoretical_data <- tibble(
  k = k,
  # Poisson distribution
  `Poisson(lambda)` = dpois(k, lambda = lambda),
  # Negative Binomial distributions: parameter order switched to (nu, lambda)
  `NB(nu=1, lambda)` = dnbinom(k, mu = lambda, size = 1),
  `NB(nu=10, lambda)` = dnbinom(k, mu = lambda, size = 10),
  `NB(nu=100, lambda)` = dnbinom(k, mu = lambda, size = 100)
) %>%
  ...
  # Set the factor order (based on the new names)
  mutate(
    Distribution = factor(Distribution, 
                          # Updated levels to reflect the (nu, lambda) order
                          levels = c("Poisson(lambda)", "NB(nu=100, lambda)", "NB(nu=10, lambda)", "NB(nu=1, lambda)"))
  )

# Define Greek/Expression Labels for Plotting
greek_labels <- c(
  expression("Poisson(" * lambda * ")"),
  # Updated expressions to show nu first
  expression("NB(" * nu * "=100, " * lambda * ")"),
  expression("NB(" * nu * "=10, " * lambda * ")"),
  expression("NB(" * nu * "=1, " * lambda * ")")
)

# Create the Plot
distributions_plot <- ggplot(theoretical_data, aes(x = k, y = Probability, color = Distribution)) +
  
  # Use geom_segment for vertical lines (the PMF)
  geom_segment(aes(xend = k, yend = 0), linewidth = 1.2, lineend = "round") + 
  
  # Add points at the top of the lines for visual emphasis
  geom_point(size = 3) + 
  
  # Set the X-axis label using expression(N)
  scale_x_continuous(
    breaks = seq(0, k_max, by = 5),
    name = expression(N) # Use N symbol for x-axis label
  ) +
  
  # Use scale_color_manual to apply the specific colors from the wes_palette function
  scale_color_brewer(
    palette = "Dark2",
    # Apply the expression labels to the legend
    labels = greek_labels
  ) +
  
  # Labels and Titles, using expressions
  labs(
    title = NULL, # Removed title
    subtitle = NULL, # Removed subtitle
    y = expression(P(N == k)), # Use P(N=k) for y-axis label
    color = "Distribution"
  ) +
...
# Save the plot
ggsave(distributions_plot, 
       file=file.path(dir, "Figures", "NB_versus_Poisson_distribution.png"),
       width=8,
       height=6)
...
head(DS <- read.table(file.path(dir, "Kery_2024", "data", "amro_obsdetection.csv"),
                      header=T,
                      sep=";",
                      as.is = T))
...
Sigma_beta <- diag(1.5^2, ncol(X))
mu_gamma <- numeric(ncol(H))
Sigma_gamma <- diag(1.5^2, ncol(H))
...
# Save ouput
save(ds_II, file=file.path(dir, "Outputs", "ds_II.RData"))
```

## Stage II (Poisson and negative binomial IDMs, implemented in stan)
...
# Check everything converged  
pHMC_fit$summary(variables = c("beta", "xi", "tau"))
...
draws_array <- as_draws_array(pHMC_fit$draws())
...
###
### Fit Poisson IDM (with Variational Inference)  
###

pVI_fit <- mod$variational(
  data = data_list,
  algorithm = "meanfield",
  output_samples = 1000,
  adapt_engaged = TRUE,
...
# Check everything converged  
pVI_fit$summary(variables = c("beta", "xi", "tau"))


# Compile the negative binomial IDM
mod <- cmdstan_model(file.path(dir, "algorithms", "IDS_NB.stan"))
...
###
### Fit negative binomial IDM (with HMC) 
###

nbHMC_fit <- mod$sample(
  data = data_list,
  chains = 4,
  parallel_chains = 4,
  iter_warmup = 200,
...
# Check everything converged  
nbHMC_fit$summary(variables = c("beta", "xi", "tau", "nu"))
...
# Convert to draws_array format
draws_array <- as_draws_array(nbHMC_fit$draws())
...
# Traceplot for nu
mcmc_trace(draws_array, pars = vars(contains("nu")))
...
# Extract all draws of 'beta' (all chains) Poisson IDM (HMC)
beta_draws_array <- pHMC_fit$draws(variables = "beta")
...
beta_pHMC <- t(beta_matrix)

# Extract beta Poisson IDM (VI)
beta_pVI <- t(pVI_fit$draws(variables = "beta"))

# Extract all draws of 'beta' (all chains) negative binomial IDM (HMC)
beta_draws_array <- nbHMC_fit$draws(variables = "beta")
# Convert to a flat matrix: (iterations * chains) × parameters
beta_matrix <- as_draws_matrix(beta_draws_array)
# transpose to get (parameters × draws)
beta_nbHMC <- t(beta_matrix)
...
###
### Create RASTER of median predictions Poisson IDM (HMC)
###
...
### Add intercept adjustment (post-hoc correction for availability)
beta_pHMC[1,] <- beta_pHMC[1,] + (adj - mean(beta_pHMC[1,]))

### Plot map
pHMC.predictions <- apply(beta_pHMC, 2, function(x) exp(X_pred %*% x)) # Make predictions for each km-square 
pHMC.predictions <- cbind(centroids, med=apply(pHMC.predictions, 1, median))
pHMC.predictions.output_sf = st_as_sf(pHMC.predictions, coords = c("X", "Y"), crs = st_crs(polygon.grid))
pHMC.predictions.output_rast <- rasterize(vect(pHMC.predictions.output_sf), test.raster, field = "med")
pHMC.predictions.output_rast <- mask(pHMC.predictions.output_rast, vect(mask)) 

# Plot density map
plot(
  pHMC.predictions.output_rast, 
  main = "p(H)",
  col = rev(heat.colors(100))
)

###
### Create RASTER of median predictions negative binomial IDM (HMC)
###
...
### Add intercept adjustment (post-hoc correction for availability)
beta_nbHMC[1,] <- beta_nbHMC[1,] + (adj - mean(beta_nbHMC[1,]))

### Plot map
nbHMC.predictions <- apply(beta_nbHMC, 2, function(x) exp(X_pred %*% x)) # Make predictions for each km-square 
nbHMC.predictions <- cbind(centroids, med=apply(nbHMC.predictions, 1, median))
nbHMC.predictions.output_sf <- st_as_sf(nbHMC.predictions, coords = c("X", "Y"), crs = st_crs(polygon.grid))
nbHMC.predictions.output_rast <- rasterize(vect(nbHMC.predictions.output_sf), test.raster, field = "med")
nbHMC.predictions.output_rast <- mask(nbHMC.predictions.output_rast, vect(mask)) 

# Plot density map
plot(
  nbHMC.predictions.output_rast, 
  main = "nb(H)",
  col = rev(heat.colors(100))
)

###
### Create RASTER of median predictions Poisson IDM (VI)
###
...
### Add intercept adjustment (post-hoc correction for availability)
beta_pVI[1,] <- beta_pVI[1,] + (adj - mean(beta_pVI[1,]))

### Plot map
pVI.predictions <- apply(beta_pVI, 2, function(x) exp(X_pred %*% x)) # Make predictions for each km-square 
pVI.predictions <- cbind(centroids, med=apply(pVI.predictions, 1, median))
pVI.predictions.output_sf <- st_as_sf(pVI.predictions, coords = c("X", "Y"), crs = st_crs(polygon.grid))
pVI.predictions.output_rast <- rasterize(vect(pVI.predictions.output_sf), test.raster, field = "med")
pVI.predictions.output_rast <- mask(pVI.predictions.output_rast, vect(mask)) 

# Plot density map
plot(
  pVI.predictions.output_rast, 
  main = "p(V)",
  col = rev(heat.colors(100))
)
...
df_pHMC <- as.data.frame(pHMC.predictions.output_rast, xy = TRUE, na.rm = TRUE) %>%
  rename(pred = 3) %>%
  mutate(model = "p(H)")

df_pVI <- as.data.frame(pVI.predictions.output_rast, xy = TRUE, na.rm = TRUE) %>%
  rename(pred = 3) %>%
  mutate(model = "p(V)")

df_nbHMC <- as.data.frame(nbHMC.predictions.output_rast, xy = TRUE, na.rm = TRUE) %>%
  rename(pred = 3) %>%
  mutate(model = "nb(H)")

# Combine into one data frame
df_combined <- 
  bind_rows(df_kery, df_GR, df_pHMC, df_pVI, df_nbHMC) %>%
  mutate(
    model = factor(model, levels=c("g(P)", "nb(H)", "p(H)", "p(V)", "Kery"))
  )
...
combined_plot <- plot_grid(
  ...
  american_robin_distribution,
  ncol = 2,
  rel_widths = c(0.51, 0.76),  # tweak as needed
...
ggsave(plot=combined_plot, 
       filename=file.path(dir, "Figures", "american_robin_distribution.png"),
       dpi=500, height=9, width=12)
```
...
gP <- 1000 / (mean(apply(beta, 1, mcmcse::ess)) / as.vector(ds_II$time))
Kery <- 1000 / (mean(apply(beta_Kery, 1, mcmcse::ess)) / as.vector(out5A$time))
pHMC <- 1000 / (mean(apply(beta_pHMC, 1, mcmcse::ess)) / as.vector(pHMC_fit$time()$total))
nbHMC <- 1000 / (mean(apply(beta_nbHMC, 1, mcmcse::ess)) / as.vector(nbHMC_fit$time()$total))
pVI <- as.vector(pVI_fit$time()$total)
...
# Create a data frame for the table
summary_df <- rbind(
  pVI   = mean_sd_format(beta_pVI),
  nbHMC = mean_sd_format(beta_nbHMC),
  pHMC  = mean_sd_format(beta_pHMC),
  gP    = mean_sd_format(beta),
  Kery  = mean_sd_format(beta_Kery)
) %>%
  cbind(
    round(c(pVI, nbHMC, pHMC, gP, Kery))
  )
