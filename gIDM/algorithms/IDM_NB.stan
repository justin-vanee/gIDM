functions {
  ...
  // --- Depletion partial sum (NB version) ---
  real depletion_lpmf_partial_sum(
    ...
    real lp = 0;
    int n_slices = end - start + 1;
    
    for (i in 1:n_slices) {
      int site = slice_idx[i];
...
model {
  ...
  // variance parameters 
  sigma2_zeta ~ inv_gamma(2, square(1.5));
  sigma2_xi ~ inv_gamma(2, square(1.5));
  sigma2_eta ~ inv_gamma(2, square(1.5));
  ...
  // Spatial Dependence (within stream network)
{
  ...
  for (i in 1:n_z) {
    real mu = exp(log_prob[i]);
    // real prob = 1 - pow(nu / (nu + mu), nu);
    real log_p0 = nu * log(nu) - nu * log(nu + mu);
    target += bernoulli_lpmf(z_flat[i] |  1 - exp(log_p0));
  }
