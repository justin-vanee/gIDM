###
### Functions 
###

logit_inv <- function(x) {
  1 / (1 + exp(-x))
}

logit <- function(p) {
  log(p / (1 - p))
}

cloglog_inv <- function(x) {1 - exp(-exp(x))}

pad_square_matrices <- function(mat_list) {
  # Determine the maximum dimension
  max_dim <- max(sapply(mat_list, nrow))  # same as ncol for square matrices
  
  # Pad each matrix
  padded_list <- lapply(mat_list, function(mat) {
    d <- nrow(mat)
    padded <- matrix(0, nrow = max_dim, ncol = max_dim)
    padded[1:d, 1:d] <- mat
    return(padded)
  })
  
  return(padded_list)
}

blockdiag_to_list <- function(D, block_sizes) {
  out <- vector("list", length(block_sizes))
  start <- 1
  for (i in seq_along(block_sizes)) {
    end <- start + block_sizes[i] - 1
    out[[i]] <- D[start:end, start:end, drop = FALSE]
    start <- end + 1
  }
  out
}

replace_low_off_diagonals <- function(mat_list, value) {
  lapply(mat_list, function(mat) {
    if (!is.matrix(mat) || nrow(mat) != ncol(mat)) {
      stop("All elements must be square matrices.")
    }
    off_diag_idx <- row(mat) != col(mat)
    mat[off_diag_idx & mat < value] <- value
    return(mat)
  })
}

plot_traceplots <- function(mcmc_mat, param_names = NULL, burnin = 0) {
  if (is.null(param_names)) {
    param_names <- paste0("beta[", seq_len(nrow(mcmc_mat)), "]")
  }
  
  # Apply burn-in trimming
  if (burnin > 0) {
    mcmc_mat <- mcmc_mat[, -(1:burnin), drop = FALSE]
  }
  
  # Convert to long format for ggplot
  library(tidyverse)
  trace_df <- as.data.frame(mcmc_mat) %>%
    mutate(Parameter = param_names) %>%
    pivot_longer(
      cols = -Parameter,
      names_to = "Iteration",
      values_to = "Value"
    ) %>%
    mutate(
      Iteration = as.integer(gsub("V", "", Iteration)) + burnin,
      Parameter = factor(Parameter, levels = param_names)
    )
  
  # Plot
  ggplot(trace_df, aes(x = Iteration, y = Value)) +
    geom_line(alpha = 0.6, color = "steelblue") +
    facet_wrap(~Parameter, scales = "free_y", ncol = 1) +
    theme_minimal() +
    labs(
      title = "Traceplots for MCMC Samples",
      x = "Iteration", y = "Sample Value"
    )
}

make_ar1_corr <- function(T, rho) {
  mat <- matrix(0, nrow = T, ncol = T)
  for (i in 1:T) {
    for (j in i:T) {
      mat[i, j] <- rho^abs(i - j)
      mat[j, i] <- mat[i, j]  # enforce symmetry
    }
  }
  return(mat)
}

fast_rmvn <- function(mean, cov) {
  L <- chol(cov)                      # Cholesky decomposition: cov = L' * L
  z <- rnorm(length(mean))           # Standard normal vector
  mean + backsolve(L, z, transpose = TRUE)  # Equivalent to: mean + L'^{-1} z
}

NB_polson <- function(N, nu, log_lambda){
  psi = logit_inv(log_lambda)
  dense = nu * log(1-psi) + lgamma(N+nu) - lgamma(nu)
  return(dense)
}

NB_polson_N <- function(N, nu, log_lambda){
  psi = logit_inv(log_lambda)
  dense = N * log(psi) + lgamma(N+nu) - lfactorial(N)
  return(dense)
}

NB_polson_II <- function(N, nu, log_lambda){
  psi = logit_inv(log_lambda)
  dense = N * log(psi) + nu * log(1-psi)
  return(dense)
}

make_taildown_spherical_R <- function(D_list, A_list, B_list, range) {
  R_list <- vector("list", length(D_list))
  
  for (i in seq_along(D_list)) {
    D <- D_list[[i]]
    A <- A_list[[i]]
    B <- B_list[[i]]
    
    # Normalize distances
    r  <- D / range
    r1 <- A / range
    r2 <- B / range
    
    # Initialize R matrix
    R <- matrix(0, nrow = nrow(D), ncol = ncol(D))
    
    # Identity matrix for diagonals
    diag(R) <- 1
    
    # Flow-connected: A == 0, D > 0
    conn_mask <- (A == 0) & (D > 0) & (r <= 1)
    R[conn_mask] <- (1 - 1.5 * r[conn_mask] + 0.5 * r[conn_mask]^3)
    
    # Flow-unconnected: A > 0, B > 0, r2 <= 1
    unconn_mask <- (A > 0) & (B > 0) & (r2 <= 1)
    R[unconn_mask] <- (1 - 1.5 * r1[unconn_mask] + 0.5 * r2[unconn_mask]) *
      (1 - r2[unconn_mask])^2
    
    # Save result
    R_list[[i]] <- R
  }
  
  return(R_list)
}

compute_crps_matrices <- function(S_test, X_test, year_mat_test, watershed_mat_test, site_mat_test,
                                  v_test, PG_out, burnin, n_mcmc) {
  
  # Helper function for linear predictor
  predict_log_lambda <- function(S, X, year, watershed, site, beta, xi, zeta, eta) {
    log(S) + X %*% beta + year %*% xi + watershed %*% zeta + site %*% eta
  }
  
  n_pred <- length(v_test)
  n_iter <- n_mcmc - burnin
  
  crps       <- matrix(NA, nrow = n_pred, ncol = n_iter)
  crps_logit <- matrix(NA, nrow = n_pred, ncol = n_iter)
  
  for (l in (burnin + 1):n_mcmc) {
    log_lambda <- predict_log_lambda(
      S_test, X_test, year_mat_test, watershed_mat_test, site_mat_test,
      PG_out$beta[, l], PG_out$xi[, l], PG_out$zeta[, l], PG_out$eta[, l]
    )
    
    idx <- l - burnin
    crps[, idx]       <- (v_test - (1 - exp(-exp(log_lambda))))^2
    crps_logit[, idx] <- (v_test - logit_inv(log_lambda))^2
  }
  
  return(list(crps = crps, crps_logit = crps_logit))
}

extract_trace_df <- function(fit_list, thin_seq, param, rows = 1:4) {
  map2_dfr(fit_list, names(fit_list), ~ {
    mat <- .x[[param]][rows, thin_seq, drop = FALSE]
    mat <- if (is.null(dim(mat))) matrix(mat, nrow = 1) else mat
    colnames(mat) <- paste0("iter_", seq_len(ncol(mat)))
    
    as_tibble(mat) %>%
      mutate(param_row = row_number()) %>%
      pivot_longer(-param_row, names_to = "iter", values_to = "value") %>%
      mutate(
        iter = as.integer(gsub("iter_", "", iter)),
        model = .y,
        param = paste0(param, "_", param_row)
      )
  })
}

extract_scalar_trace_df <- function(fit_list, thin_seq, param) {
  map2_dfr(fit_list, names(fit_list), ~ {
    vec <- .x[[param]][thin_seq]
    tibble(
      iter = seq_along(vec),
      value = vec,
      model = .y,
      param = param
    )
  })
}

plot_trace_df <- function(df, burnin = 4000) {
  df %>%
    filter(iter > burnin) %>%
    ggplot(aes(x = iter, y = value)) +
    geom_line(alpha = 0.5) +
    facet_grid(param ~ model, scales = "free_y") +
    theme_minimal() +
    labs(x = "Iteration", y = "Value", title = "Traceplots by Parameter and Model")
}

extract_log_lambda_means <- function(fit_list) {
  map2_dfr(fit_list, names(fit_list), ~ {
    log_lambda_sub <- .x$log_lambda[, thin_seq, drop = FALSE]
    means <- rowMeans(log_lambda_sub)
    tibble(
      value = means,
      model = .y
    )
  })
}

###
### Plotting Functions
###

generate_traceplots <- function(model_fits, thin_seq, burnin, rows, xlim=c(-5,10)) {
  
  # Check if the required 'joint' model is present
  if (!any(str_detect(names(model_fits), "joint"))) {
    stop("The 'joint' model is required in model_fits but was not found.")
  }
  
  # List to store all generated plots
  plots <- list()
  
  # --- 1. MCMC Trace Plots ---
  
  # Define the variables, required rows, and required model list for each plot
  plot_configs <- list(
    
    # --- Abundance Parameters (beta) ---
    # Variable: beta (Abundance Regression Coefficients)
    beta = list(var = "beta", rows = rows, models = names(model_fits), 
                plot_name = "Trace_Beta"),
    
    # Variable: sigma2_xi (temporal variance)
    sigma2_xi = list(var = "sigma2_xi", rows = NULL, models = names(model_fits), 
                  plot_name = "Trace_sigma2_xi", is_scalar = TRUE),
    
    # Variable: sigma2_eta (spatial variance (network))
    sigma2_eta = list(var = "sigma2_eta", rows = rows, models = names(model_fits), 
                     plot_name = "Trace_sigma2_eta"),
    
    # Variable: sigma2_zeta (spatial variance (watershed))
    sigma2_zeta = list(var = "sigma2_zeta", rows = NULL, models = names(model_fits), 
                     plot_name = "Trace_sigma2_zeta", is_scalar = TRUE),
    
    # --- Detection Parameters (gamma) ---
    # Variable: gamma (Detection Regression Coefficients) - Excludes binary model
    gamma = list(var = "gamma", rows = rows, models = names(model_fits)[-2], # no binary
                 plot_name = "Trace_Gamma"),
    
    # Variable: phi (Spatial Autocorrelation)
    phi = list(var = "phi", rows = rows, models = names(model_fits), 
                 plot_name = "Trace_alpha"),
    
    # Variable: rho (Temporal Autocorrelation)
    rho = list(var = "rho", rows = NULL, models = names(model_fits), 
               plot_name = "Trace_rho", is_scalar = TRUE),
    
    # Variable: xi (Temporal RE) 
    xi = list(var = "xi", rows = rows, models = names(model_fits), 
                plot_name = "Trace_Xi"),
    
    # Variable: xi (Spatial RE) 
    zeta = list(var = "zeta", rows = rows, models = names(model_fits), 
              plot_name = "Trace_zeta"),
    
    # Variable: eta (Spatial RE) 
    eta = list(var = "eta", rows = rows, models = names(model_fits), 
              plot_name = "Trace_eta"),
    
    # --- Abundance Process (N) ---
    # Variable: N (Abundance) - Excludes binary model (-2 is the index of binary)
    N = list(var = "N", rows = rows, models = names(model_fits)[-2], # [1] and [3] are count, joint
             plot_name = "Trace_N")
  )
  
  # Loop through configurations to generate trace plots
  for (config in plot_configs) {
    var_name <- config$var
    plot_name <- config$plot_name
    
    # Conditional check: Ensure the variable exists in the 'joint' model
    if (var_name %in% names(model_fits$joint)) {
      
      # Filter model fits to only include required models for this variable (e.g., N excludes binary)
      filtered_fits <- model_fits[config$models]
      
      if (isTRUE(config$is_scalar)) {
        # Use extract_scalar_trace_df for scalar parameters
        df <- extract_scalar_trace_df(filtered_fits, thin_seq=thin_seq, var_name)
      } else {
        # --- ROBUST BOUNDARY CHECK FOR MATRIX PARAMETERS ---
        requested_rows <- config$rows
        
        # Determine the actual number of rows available for this parameter from the first fit
        first_fit <- filtered_fits[[1]]
        param_data <- first_fit[[var_name]]
        n_rows <- 0 # Default if data is not available or not a matrix
        
        if (!is.null(param_data) && (is.matrix(param_data) || is.array(param_data))) {
          n_rows <- dim(param_data)[1]
          
          # Filter the requested rows to keep only valid indices (>= 1 and <= n_rows)
          safe_rows <- requested_rows[requested_rows >= 1 & requested_rows <= n_rows]
          
        } else {
          # If the parameter is not a matrix/array, we cannot apply row subsetting, 
          # so we'll rely on the warning check below.
          safe_rows <- requested_rows
          warning(paste0("Parameter '", var_name, "' in model '", names(filtered_fits)[1], 
                         "' is not a matrix/array. Attempting to use requested rows."))
        }
        
        if (length(safe_rows) == 0 && !is.null(requested_rows)) {
          warning(paste0("Skipping plot '", plot_name, 
                         "' (", var_name, ") as no valid rows were found within the bounds (Max rows: ", 
                         n_rows, ")."))
          next # Skip to the next plot configuration
        }
        
        # Use extract_trace_df with the calculated safe rows
        df <- extract_trace_df(filtered_fits, thin_seq=thin_seq, var_name, rows = safe_rows)
        # --- END OF BOUNDARY CHECK ---
      }
      
      # Generate the plot
      plots[[plot_name]] <- plot_trace_df(df, burnin)
      
    } else {
      warning(paste("Variable '", var_name, "' not found in the 'joint' model. Skipping trace plot.", sep=""))
    }
  }
  
  # --- 2. Mean Abundance Histogram Plot (log_lambda) ---
  
  # Check if 'log_lambda' is available in the joint model
  if ("log_lambda" %in% names(model_fits$joint)) {
    
    # Use the external function to process log_lambda means
    log_lambda_df <- extract_log_lambda_means(model_fits)
    
    # Create the histogram plot
    log_lambda_hist_plot <- ggplot(log_lambda_df, aes(x = value)) +
      geom_histogram(bins = 30, fill = "steelblue", alpha = 0.7) +
      facet_wrap(~ model, scales = "free_y") +
      theme_minimal() +
      labs(x = expression("Mean log(" * lambda * ")"), 
           y = "Count", 
           title = "Predicted Log Density by Model") +
      xlim(xlim[1], xlim[2])
    
    plots[["Histogram_Log_Lambda"]] <- log_lambda_hist_plot
    
  } else {
    warning("Variable 'log_lambda' not found in the 'joint' model. Skipping histogram plot.")
  }
  
  return(plots)
}

## Simulate count and presence/absence data from shared process
simulate_data <- 
  function(n, n_ppd, z_prop, p, nu=NULL){
    
    # Calculate number of absence/presence sites
    n_tot = n + n_ppd
    
    # Calculate number of absence/presence sites
    n_z <- round(z_prop * n)
    n_N <- n - n_z
    z_idx <- 1:n_z
    
    # Simulate n random points in (0,1) x (0,1)
    coords <- matrix(runif(2 * n_tot), ncol = 2)
    
    # Compute the Euclidean distance matrix
    D_tot <- as.matrix(dist(coords))
    
    # Matrices 
    X_tot <- 
      cbind(
        rep(1, n_tot),
        matrix(rnorm(p * n_tot), n_tot, p)
      )
    
    # Parameters values 
    sigma2 = rgamma(1, 1, 1)
    phi = runif(1)
    beta = rnorm(p+1)
    
    # Simulate eta * log_lambda
    R_tot = exp( -D_tot / phi )
    eta_tot = rmvn(1, numeric(n_tot), sigma2 * R_tot)
    eta_tot = eta_tot - mean(eta_tot) # post center to mean 0 
    log_lambda_tot = X_tot %*% beta + eta_tot
    lambda_tot = exp(log_lambda_tot)
    
    # Simulate observations 
    if(is.null(nu)){
      N_latent = rpois(n_tot, lambda_tot)
      # Observed counts 
      N = N_latent[1:n][-z_idx]
      # Censored data 
      z = as.numeric(N_latent[z_idx] > 0)
      # out-of-sample 
      N_ppd = N_latent[-(1:n)]
    } else {
      # Typical mean parameterization 
      N_latent = rnbinom(n_tot, size=nu, mu=lambda_tot) 
      # Observed counts 
      N = N_latent[1:n][-z_idx]
      # Censored data 
      z = as.numeric(N_latent[z_idx] > 0)
      # out-of-sample 
      N_ppd = N_latent[-(1:n)]
    }
    
    # ---- Filter for observed and conditional matrices ----
    eta = eta_tot[1:n]
    log_lambda = log_lambda_tot[1:n]
    X = X_tot[1:n,]
    D = D_tot[1:n, 1:n]
    X_ppd = X_tot[-c(1:n),]
    R_ppd = R_tot[-(1:n), -(1:n)] - R_tot[-(1:n), (1:n)] %*% solve( R_tot[(1:n), (1:n)] ) %*% R_tot[(1:n), -(1:n)]
    R_cond = R_tot[-(1:n), (1:n)] %*% solve( R_tot[(1:n), (1:n)] )
    
    # ---- Assemble list ----
    return(
      list(
        n = n,
        n_ppd = n_ppd, 
        p = p+1, # +1 for intercept
        n_z = n_z,
        n_N = n_N,
        coords = coords,
        D = D,
        X = X,
        X_ppd = X_ppd,
        R_ppd = R_ppd,
        R_cond = R_cond, 
        z = z,
        N = N,
        N_ppd = N_ppd,
        # Parameter values
        phi = phi,
        sigma2 = sigma2,
        beta = beta,
        eta = eta, 
        log_lambda = log_lambda,
        nu = nu
      )
    )
    
  }

## Fit simulation models 
fit_models <- 
  function(data,
           iter_sampling, iter_warmup, chains=1,
           n_mcmc, burnin,
           VI_iter, VI_draws,
           dir, 
           Models = c("p(HMC)", "p(VI)", "nb(HMC)", "g(PG)")) {
    
    # Create tibble for outputs
    metrics_tib <- tibble::tibble(
      Model = Models,
      dataset = data$dataset,
      time = NA,
      cov_beta = NA,
      cov_eta = NA,
      cov_N = NA, 
      bias_beta = NA,
      bias_eta = NA,
      bias_N = NA, 
      sd_beta = NA,
      sd_eta = NA,
      sd_N = NA, 
      ess_beta = NA,
      ess_eta = NA,
      ess_N = NA, 
      nu_mean = NA, 
      nu = data$nu
    ) %>% tibble::column_to_rownames("Model")
    
    if(is.null(data$nu)){
      data$nu=NULL
    }
    
    # Fit Poisson-IDM with Hamiltonian Monte Carlo 
    if ("p(HMC)" %in% Models) {
  
    model <- cmdstan_model(
      file.path(dir, "algorithms", "IDM_sim.stan")
    )
  
    fit <- 
      model$sample(
        data = data,
        chains = chains,
        iter_sampling = iter_sampling,
        iter_warmup = iter_warmup
      )
  
    # --- check success here ---  
    if (all(fit$return_codes() == 0)) {
    
      metrics_tib["p(HMC)", "time"] <- pluck(fit$time(), "total")
    
      # Extract posterior samples (HMC) 
      HMC_posterior_beta <-
        fit$draws(variables = c("beta")) %>%
        aperm(c(3, 1, 2)) %>%
        matrix(nrow = data$p, chains * iter_sampling)
      
      HMC_posterior_eta <-
        fit$draws(variables = c("eta")) %>%
        aperm(c(3, 1, 2)) %>%
        matrix(nrow = data$n, chains * iter_sampling)
      
      # Calculate posterior predictive distribution for N
      HMC_ppd_N = matrix(NA, data$n_ppd, iter_sampling)
      cond_mean = data$X_ppd %*% HMC_posterior_beta + data$R_cond %*% HMC_posterior_eta
      sigma2 = as.vector(fit$draws(variables = c("sigma2"))) 
      for(i in 1:iter_sampling){
        cond_var = sigma2[i] * data$R_ppd
        log_lambda = rmvn(1, mu = cond_mean[, i], sigma = cond_var)
        HMC_ppd_N[, i] = rpois(data$n_ppd, exp(log_lambda))
      }
    
    } else {
      
      message("⚠️ Poisson HMC failed (non-zero return code). Leaving metrics as NA.")
      HMC_posterior_beta <- NULL
      HMC_posterior_eta <- NULL
      HMC_ppd_N <- NULL
      
      }
    }
    
    # Fit Negative-Binomial-IDM with Hamiltonian Monte Carlo 
    if ("nb(HMC)" %in% Models) {
      
      model <- cmdstan_model(
        file.path(dir, "algorithms", "IDM_sim_NB.stan")
      )
      
      fit <- 
        model$sample(
          data = data,
          chains = chains,
          iter_sampling = iter_sampling,
          iter_warmup = iter_warmup
        )
      
      # --- check success here ---  
      if (all(fit$return_codes() == 0)) {
        
        metrics_tib["nb(HMC)", "time"] <- pluck(fit$time(), "total")
        
        # Extract posterior samples (HMC) 
        NB_posterior_beta <-
          fit$draws(variables = c("beta")) %>%
          aperm(c(3, 1, 2)) %>%
          matrix(nrow = data$p, chains * iter_sampling)
        
        NB_posterior_eta <-
          fit$draws(variables = c("eta")) %>%
          aperm(c(3, 1, 2)) %>%
          matrix(nrow = data$n, chains * iter_sampling)
        
        # Calculate posterior predictive distribution for N
        NB_ppd_N = matrix(NA, data$n_ppd, iter_sampling)
        cond_mean = data$X_ppd %*% NB_posterior_beta + data$R_cond %*% NB_posterior_eta
        sigma2 = as.vector(fit$draws(variables = c("sigma2"))) 
        nu = as.vector(fit$draws(variables = c("nu"))) 
        for(i in 1:iter_sampling){
          cond_var = sigma2[i] * data$R_ppd
          log_lambda = rmvn(1, mu = cond_mean[, i], sigma = cond_var)
          NB_ppd_N[, i] = rnbinom(data$n_ppd, size = nu[i], mu = exp(log_lambda))
        }
        
        # Extract posterior mean of nu 
        metrics_tib["nb(HMC)", "nu_mean"] <- mean(fit$draws(variables = c("nu")))
        
      } else {
        
        message("⚠️ Negative-Binomial HMC failed (non-zero return code). Leaving metrics as NA.")
        NB_posterior_beta <- NULL
        NB_posterior_eta <- NULL
        NB_ppd_N <- NULL
        
      }
    }

    # Fit Geometric IDM 
    if ("g(PG)" %in% Models) {
      source(file.path(dir, "algorithms", "MCMC.R"))
      mu_beta <- numeric(data$p)
      Sigma_beta <- diag(1.5^2, data$p)
      
      Start <- Sys.time()
      gIDM_out <- gIDM_sim(
        z = data$z, N = data$N, X = data$X, D = data$D,
        mu_beta = mu_beta, Sigma_beta = Sigma_beta,
        phi = data$phi,
        n_mcmc = n_mcmc
      )
      End <- Sys.time()
      metrics_tib["g(PG)", "time"] <- difftime(End, Start, units = "secs")
      
      # Calculate posterior predictive distribution for N
      g_ppd_N = matrix(NA, data$n_ppd, iter_sampling)
      cond_mean = data$X_ppd %*% gIDM_out$beta[,-c(1:burnin)] + data$R_cond %*% gIDM_out$eta[,-c(1:burnin)]
      sigma2 = gIDM_out$sigma2
      for(i in 1:iter_sampling){
        cond_var = sigma2[i] * data$R_ppd
        log_lambda = rmvn(1, mu = cond_mean[, i], sigma = cond_var)
        g_ppd_N[, i] = rgeom(data$n_ppd,  prob = 1 - logit_inv(log_lambda))
      }
    }
    
    # Fit Poisson-IDM with variational inference (mean field approximation)
    if ("p(VI)" %in% Models) {
      if (!exists("model")) {
        model <- cmdstan_model(
          file.path(dir, "algorithms", "IDM_sim.stan")
        )
      }
      
      # Run Pathfinder (other VI will often fail)
      pathfinder_fit <- model$pathfinder(
        data = data,
        num_paths = 1
      )
      
      # Extract good initial values
      init_list <- pathfinder_fit$draws(format = "list")
      
      fit_VI <-
        model$variational(
        data = data,
        algorithm = "meanfield",
        iter = VI_iter,
        init = init_list,
        draws = VI_draws
      )
      
      # --- check success here ---
      if (all(fit_VI$return_codes() == 0)) {
        metrics_tib["p(VI)", "time"] <- pluck(fit_VI$time(), "total")
        
        VI_posterior_beta <-
          fit_VI$draws(variables = "beta") %>% t()
        
        VI_posterior_eta <-
          fit_VI$draws(variables = "eta") %>% t()
        
        # Calculate posterior predictive distribution for N
        VI_ppd_N = matrix(NA, data$n_ppd, iter_sampling)
        cond_mean = data$X_ppd %*% VI_posterior_beta + data$R_cond %*% VI_posterior_eta
        sigma2 = as.vector(fit_VI$draws(variables = c("sigma2"))) 
        for(i in 1:iter_sampling){
          cond_var = sigma2[i] * data$R_ppd
          log_lambda = rmvn(1, mu = cond_mean[, i], sigma = cond_var)
          VI_ppd_N[, i] = rpois(data$n_ppd, exp(log_lambda))
        }
        
        
      } else {
        message("⚠️ VI failed (non-zero return code). Leaving metrics as NA.")
        VI_posterior_beta <- NULL
        VI_posterior_eta <- NULL
        VI_ppd_N <- NULL
      }
      
    }
    
    # Define thinning indices
    data$beta <- data$beta[-1] # Remove intercept
    
    # Loop over models and compute metrics
    for (model_name in Models) {
      if (model_name == "p(HMC)" && !is.null(HMC_posterior_beta)) {
        beta_post <- HMC_posterior_beta[-1,] # Remove intercept
        eta_post <- HMC_posterior_eta
        N_ppd <- HMC_ppd_N
      } else if (model_name == "nb(HMC)") {
        beta_post <- NB_posterior_beta[-1,] # Remove intercept
        eta_post <- NB_posterior_eta
        N_ppd <- NB_ppd_N
      } else if (model_name == "g(PG)") {
        beta_post <- gIDM_out$beta[-1, -c(1:burnin)] # Remove intercept
        eta_post <- gIDM_out$eta[, -c(1:burnin)]
        N_ppd <- g_ppd_N
      } else if (model_name == "p(VI)" && !is.null(VI_posterior_beta)) {
        beta_post <- VI_posterior_beta[-1,] # Remove intercept
        eta_post <- VI_posterior_eta
        N_ppd <- VI_ppd_N
      } else {
        next
      }
      
      # Coverage
      lwr <- apply(eta_post, 1, quantile, 0.05)
      upr <- apply(eta_post, 1, quantile, 0.95)
      metrics_tib[model_name, "cov_eta"] <- mean(lwr <= data$eta & data$eta <= upr)
      
      lwr <- apply(beta_post, 1, quantile, 0.05)
      upr <- apply(beta_post, 1, quantile, 0.95)
      metrics_tib[model_name, "cov_beta"] <- mean(lwr <= data$beta & data$beta <= upr)
      
      lwr <- apply(N_ppd, 1, quantile, 0.05)
      upr <- apply(N_ppd, 1, quantile, 0.95)
      metrics_tib[model_name, "cov_N"] <- mean(lwr <= data$N_ppd & data$N_ppd <= upr)
      
      # Bias
      metrics_tib[model_name, "bias_eta"]  <- mean(abs(data$eta - rowMeans(eta_post)))
      metrics_tib[model_name, "bias_beta"] <- mean(abs(data$beta - rowMeans(beta_post)))
      metrics_tib[model_name, "bias_N"]    <- mean(abs(data$N_ppd - rowMeans(N_ppd)))
      
      # Posterior SD
      metrics_tib[model_name, "sd_eta"]  <- mean(apply(eta_post, 1, sd))
      metrics_tib[model_name, "sd_beta"] <- mean(apply(beta_post, 1, sd))
      metrics_tib[model_name, "sd_N"]    <- mean(apply(N_ppd, 1, sd))
      
      # ESS
      metrics_tib[model_name, "ess_eta"]  <- mean(apply(eta_post, 1, mcmcse::ess))
      metrics_tib[model_name, "ess_beta"] <- mean(apply(beta_post, 1, mcmcse::ess))
      metrics_tib[model_name, "ess_N"]    <- mean(apply(N_ppd, 1, mcmcse::ess), na.rm=TRUE)
    }
    
    # Clear up memory 
    fit_VI=fit=gIDM_out=NULL
    N_ppd=beta_post=eta_post=VI_posterior_beta=VI_posterior_eta=HMC_posterior_beta=HMC_posterior_eta=NULL
    gc()
    
    # Return final tibble with "Model" column restored
    metrics_tib <- metrics_tib %>% tibble::rownames_to_column("Model")
    return(metrics_tib)
  }


## Fit 3 models to real data 
fit_models_real_parallel <- 
  function(n_mcmc = 2e4, c_1 = 0.7, n_cores = 3, dir) {
    
  # Package   
  library(tidyverse)
  library(scoringRules)
  library(Matrix)
  library(foreach)
  library(doParallel)
  
  # Load data
  load(file.path(dir, "Outputs", "data.RData"))
  
  #
  # Prepare data
  #
  
  mu_beta     <- numeric(data$p_X)
  Sigma_beta  <- diag(1.5^2, data$p_X)
  mu_gamma    <- numeric(data$p_H)
  Sigma_gamma <- diag(1.5^2, data$p_H)
  
  year_idx <- c(data$z_year_idx, data$y_year_idx)
  site_idx <- c(data$z_site_idx, data$y_site_idx)
  zeta_idx <- c(data$zeta_idx[data$z_site_idx], data$zeta_idx[data$y_site_idx])
  
  watershed_levels <- sort(unique(zeta_idx))
  year_levels      <- sort(unique(year_idx))
  site_levels      <- sort(unique(site_idx))
   
  A <- data$A[watershed_levels, watershed_levels]
  X <- rbind(data$X_z_flat, data$X_y_flat)
  S <- c(data$A_z_flat, data$A_y_flat)
  
  year_mat      <- model.matrix(~ factor(year_idx, levels = year_levels) - 1)
  watershed_mat <- model.matrix(~ factor(zeta_idx, levels = watershed_levels) - 1)
  site_mat      <- model.matrix(~ factor(site_idx, levels = site_levels) - 1)
  
  # Define function call
  
  model_params <- list(
    z = data$z_flat, y = data$y_flat, J_y = data$J_y_flat, y_idx = data$y_idx, 
    y_sum = data$y_sum_flat, N_deplete = data$N_deplete_flat,
    H = data$H_y_flat, X = X, D = data$D_obs_list, A = A, S = S,
    year_mat = year_mat, watershed_mat = watershed_mat, site_mat = site_mat,
    mu_gamma = mu_gamma, Sigma_gamma = Sigma_gamma,
    mu_beta = mu_beta, Sigma_beta = Sigma_beta,
    var_shape = 2, var_rate = 1.5^2, c_1 = c_1, n_mcmc = n_mcmc
  )
  
  # Define models to fits 
  
  model_configs <- list(
    list(binary = FALSE, count = TRUE),   
    list(binary = TRUE, count = FALSE),
    list(binary = TRUE, count = TRUE)
  )
  
  # Fit models in parallel
  
  cl <- makeCluster(n_cores)
  registerDoParallel(cl)
  
  model_fits <- foreach(i = 1:3, .packages = c("Matrix")) %dopar% {
    source(file.path(dir, "algorithms", "MCMC.R"))
    
    start <- Sys.time()
    fit <- do.call(gIDM, c(model_params, model_configs[[i]]))
    end <- Sys.time()
    
    list(fit = fit, time = difftime(end, start, units = "mins"))
  }
  
  stopCluster(cl)
  
  time <- sapply(model_fits, function(x) x$time)
  
  return(list(
    count = model_fits[[1]]$fit, 
    binary = model_fits[[2]]$fit, 
    joint = model_fits[[3]]$fit, 
    time = time
  ))
}

###
### MCMC Algorithms
###

###
### Simulation Study
###

## Geometric integrated distribution model (simulation study)
gIDM_sim <- 
  function(
    # Data
    z, N, 
    X, D,
    # Priors 
    mu_beta, Sigma_beta,
    var_shape=2, var_rate=1.5^2, phi, 
    # Hyperparameters 
    n_mcmc){
    
    ###
    ### Packages
    ###
    
    library(BayesLogit)
    library(Boom)
    library(Matrix)
    
    ###
    ### Loop Variables
    ###
    
    ### Dimensions 
    n_z = length(z)
    n_N = length(N)
    p = ncol(X)
    n = nrow(X)
    
    ### Priors 
    Sigma_beta_inv = solve(Sigma_beta)
    Sigma_beta_inv_times_mu = Sigma_beta_inv %*% mu_beta
    
    ### Objects 
    kappa = z - rep(1, n_z) / 2
    I_n = diag(1, n)
    q_d = n / 2 + var_shape
    X_N = X[-(1:n_z),]
    M = cbind(X, I_n)
    M_z = M[1:n_z,]
    M_N = M[-(1:n_z),]
    mean_vec = c(Sigma_beta_inv_times_mu, numeric(n))
    
    ###
    ### Starting Values
    ###
    
    beta = mu_beta
    eta = numeric(n)
    log_lambda = numeric(n)
    sigma2 = 1
    nu = 1
    R = exp( -D / phi)
    R_inv = solve(R)
    
    ###
    ### Storage
    ###
    
    log_lambda_save = matrix(NA, n, n_mcmc)
    beta_save = matrix(NA, p, n_mcmc)
    eta_save = matrix(NA, n, n_mcmc)
    N_latent_save = matrix(NA, n_z, n_mcmc)
    sigma2_save = numeric(n_mcmc)
    
    ###
    ### MCMC loop
    ###
    
    for (q in 1:n_mcmc) {
      
      ### Sample omegas
      omega1 = rpg(n_z, 1, log_lambda[1:n_z])
      omega2 = rpg(n_N, N+nu, log_lambda[-(1:n_z)])
      
      ### Sample beta & eta
      var_inv = as.matrix(bdiag(Sigma_beta_inv, (1 / sigma2) * R_inv))
      tmp_chol = chol(t(M_z) %*% (omega1 * M_z) + t(M_N) %*% (omega2 * M_N) +  var_inv)
      coef = backsolve(tmp_chol, 
                       backsolve(tmp_chol,  
                                 t(M_z) %*% kappa + t(M_N) %*% (N-nu)/2 + mean_vec,
                                 transpose=TRUE) + rnorm(p+n)) 
      log_lambda = M %*% coef
      beta = coef[1:p]
      eta = coef[-c(1:p)]
      ### Post center eta 
      eta_mean = mean(eta)
      beta[1] = beta[1] + eta_mean
      eta = eta - eta_mean
      
      ### Sample sigma2
      r_tilde = t(eta) %*% R_inv %*% eta / 2 + var_rate
      sigma2 = 1 / rgamma(1, shape = q_d, scale = 1 / r_tilde)
      
      ### Save Samples
      log_lambda_save[, q] = log_lambda
      beta_save[, q] = beta
      eta_save[, q] = eta
      sigma2_save[q] = sigma2
      
      ### Timer
      if (q %% 1000 == 0) cat(q, " ")
    }
    
    list(
      ## MCMC samples
      log_lambda=log_lambda_save,
      beta=beta_save,
      eta=eta_save,
      sigma2=sigma2_save
    )
  }

###
### Kery et al. (2024) IDS
###

## Distance-sampling model only (Stage I)
ds_MCMC <- function(
  # Data
  Y, D, H, 
  # Priors 
  u_d, 
  mu_gamma, Sigma_gamma,
  # Hyperparameters 
  c_1=0.7, n_mcmc){   
  
  ####  
  #### Setup Variables  
  ####  
  
  M <- nrow(Y) 
  n_N <- ncol(Y)
  
  # Storage
  gamma_save <- matrix(NA, nrow=ncol(H), ncol=n_mcmc)
  chi_save <- matrix(NA, nrow=n_N, ncol=n_mcmc)
  N_save <- matrix(NA, nrow=n_N, ncol=n_mcmc)
  
  #### Starting Values
  gamma <- glm(colSums(Y) ~ H - 1, family = "poisson")$coefficients
  sigma2 <- t(exp(H %*% gamma))
  V <- Y
  D_miss <- is.na(D)
  n_D_miss <- sum(D_miss)
  D[D_miss] <- runif(n_D_miss, 0, u_d)
  chi <- colMeans(Y)
  Chi <- matrix(rep(chi, each = M), nrow = M)
  P <- exp(-sweep(D^2, 2, sigma2, FUN = "/"))
  
  #### Tuning
  gamma_acc = 0
  gamma_tune = 2.4^2 / 100
  Sigma_tune = diag(1, ncol(H))
  ### Hyperparameters for tuning adaptive sampler 
  r_opt = 0.234
  
  #### Begin MCMC Loop
  for (q in 1:n_mcmc) {
    
    ### For automatic tuning 
    autotune = 1.0 / q^c_1
    
    #### Sample V
    chi_tmp <- Chi * (1 - P) / (Chi * (1 - P) + 1 - Chi)
    V[Y == 0] <- rbinom(n_D_miss, 1, chi_tmp[Y == 0])
    
    #### Sample chi
    tmp_sum <- apply(V, 2, sum)
    chi <- rbeta(n_N, tmp_sum + 1, M - tmp_sum + 1) # Discrete-Uniform(0,M) implied for N
    Chi <- matrix(rep(chi, each = M), nrow = M)
    
    #### Sample gamma
    gamma_star <- Boom::rmvn(1, gamma, gamma_tune * Sigma_tune)
    P_star <- exp(-sweep(D^2, 2, exp(H %*% gamma_star), FUN = "/"))
    mh1 <- sum(dbinom(Y[V == 1], 1, P_star[V == 1], log = TRUE))
    mh2 <- sum(dbinom(Y[V == 1], 1, P[V == 1], log = TRUE))
    mh <- exp(mh1 - mh2)
    if (mh > runif(1)) {
      gamma <- gamma_star
      P <- P_star
      sigma2 <- t(exp(H %*% gamma))
      gamma_acc <- gamma_acc + 1
      gamma_tune <- exp(log(gamma_tune) + autotune*(1.0 - r_opt))
    } else {
      gamma_tune <- exp(log(gamma_tune) + autotune*(0.0 - r_opt))
    }
    
    #### Sample d
    D_star <- D
    D_star[D_miss] <- runif(n_D_miss, 0, u_d)
    p_star <- exp(-sweep(D_star^2, 2, sigma2, FUN = "/")[D_miss])
    mh1 <- V[D_miss] * log(1 - p_star)
    mh2 <- V[D_miss] * log(1 - P[D_miss])
    keep_idx <- exp(mh1 - mh2) > runif(n_D_miss)
    keep_idx[is.na(keep_idx)] <- FALSE # Fixes numerical issues
    D[D_miss][keep_idx] <- D_star[D_miss][keep_idx]
    P[D_miss][keep_idx] <- p_star[keep_idx]
    
    #### Save Samples
    gamma_save[,q] <- gamma
    chi_save[,q] <- chi
    N_save[,q] <- colSums(V)
    
    #### Update covariance tuning matrix
    if(q>=1000){
      Sigma_tune <- Sigma_tune + autotune * (cov(t(gamma_save[,1:q])) - Sigma_tune)
    }
    
    #### Timer
    if (q %% 1000 == 0) cat(q, " ")
  }
  
  #### Write Output
  list(
    gamma = gamma_save,
    chi = chi_save,
    N = N_save,
    gamma_acc = gamma_acc,
    Sigma = gamma_tune * Sigma_tune
  )
}


## Joint model (Stage II)
ds_II_MCMC <- 
  function(
    # Data
    z, N_I, X, S, year_mat,
    # Priors 
    mu_beta, Sigma_beta,
    var_shape=2, var_rate=1.5^2,
    # Hyperparameters 
    n_mcmc){
    
    ###
    ### Packages
    ###
    
    library(BayesLogit)
    library(Boom)
    library(Matrix)
    
    ###
    ### Loop Variables
    ###
    
    ### Dimensions 
    n_z = length(z)
    n_N = nrow(N_I)
    n_samp = ncol(N_I)
    p_X = ncol(X)
    T = ncol(year_mat)
    
    ### Priors 
    Sigma_beta_inv = solve(Sigma_beta)
    Sigma_beta_inv_times_mu = Sigma_beta_inv %*% mu_beta
    ### How we effectively set the coefficient on area (S) to 1 
    coef_precision = 1e5
    
    ### Objects 
    kappa = z - rep(1, n_z) / 2
    I = diag(1, n_z)
    I_T = diag(1, T)
    M = cbind(log(S), X, year_mat)
    M_z = M[1:n_z,]
    M_N = M[-(1:n_z),]
    
    ###
    ### Starting Values & Loop Variables
    ###
    
    q_xi = T / 2 + var_shape
    beta = mu_beta
    xi = numeric(T)
    log_lambda = numeric(n_z+n_N)
    N = N_I[,n_samp]
    tau = 1
    nu = 1
    mean_vec = c(coef_precision, Sigma_beta_inv_times_mu, rep(0, T))
    coef_inv = diag(coef_precision, 1)
    
    ###
    ### Acceptance
    ###
    
    N_acc = numeric(n_N)
    
    ###
    ### Storage
    ###
    
    beta_save = matrix(NA, p_X, n_mcmc)
    N_save = matrix(NA, n_N, n_mcmc)
    xi_save = matrix(NA, T, n_mcmc)
    log_lambda_save = matrix(NA, n_z+n_N, n_mcmc)
    tau_save = numeric(n_mcmc)
    coef_save = numeric(n_mcmc)
    
    ###
    ### MCMC loop
    ###
    
    for (q in 1:n_mcmc) {
      
      ###
      ### Sample fixed and random effects 
      ###
      
      ### Sample omegas
      omega1 = rpg(n_z, 1, log_lambda[1:n_z])
      omega2 = rpg(n_N, N+nu, log_lambda[-(1:n_z)])
      
      ### Sample beta & eta
      var_inv = as.matrix(bdiag(coef_inv, Sigma_beta_inv, (1 / tau) * I_T))
      tmp_chol = chol(t(M_z) %*% (omega1 * M_z) + t(M_N) %*% (omega2 * M_N) +  var_inv)
      
      coef = backsolve(tmp_chol, 
                       backsolve(tmp_chol,  
                                 t(M_z) %*% kappa + t(M_N) %*% (N-nu)/2 + mean_vec,
                                 transpose=TRUE) + rnorm(p_X + T + 1)) 
      log_lambda = M %*% coef
      coef_save[q] = coef[1]
      coef = coef[-1]
      beta = coef[1:p_X]
      xi = coef[(p_X + 1):(p_X + T)]
      
      ### Sample tau
      r_tilde = t(xi) %*% xi / 2 + var_rate
      tau = 1 / rgamma(1, shape = q_xi, scale = 1 / r_tilde)
      
      ###
      ### Sample N (refine Stage I samples)
      ###
      
      for(i in 1:n_N){
        
        # Proposal value
        N_star = N_I[i,sample(n_samp, 1, TRUE)]
        
        # Metropolis-Hastings ratio
        mh1 = NB_polson_N(N_star, nu, log_lambda[i+n_z])
        mh2 = NB_polson_N(N[i], nu, log_lambda[i+n_z])
        mh = exp(mh1 - mh2)
        
        # Accept or reject
        if (runif(1) < mh) {
          N[i] = N_star
          N_acc[i] = N_acc[i] + 1
        }
        
      }
      
      ### Save Samples
      log_lambda_save[, q] = log_lambda
      N_save[, q] = N
      beta_save[, q] = beta
      xi_save[, q] = xi
      tau_save[q] = tau

      ### Timer
      if (q %% 1000 == 0) cat(q, " ")
    }
    
    
    list(
      ## MCMC samples
      coef=coef_save,
      log_lambda=log_lambda_save,
      N=N_save,
      beta=beta_save,
      xi=xi_save,
      tau=tau_save
    )
  }

###
### Brook Trout (depletion sampling + presence/absence)
###

## Pólya-gamma augmentation 
gIDM <- 
  function(
    # Data
    z, y, J_y, y_idx, y_sum, N_deplete,
    H, X, D, A, S, 
    year_mat, watershed_mat, site_mat,
    # Priors 
    mu_gamma, Sigma_gamma,
    mu_beta, Sigma_beta,
    var_shape=2, var_rate=1.5^2,
    phi_shape=0.1, phi_rate=0.0001,
    # Hyperparameters 
    c_1, n_mcmc,
    # Data sources
    binary=TRUE, count=TRUE){
    
    ###
    ### Packages
    ###
    
    library(BayesLogit)
    library(Boom)
    library(Matrix)
    library(purrr)
    
    ###
    ### Loop Variables
    ###
    
    ### Dimensions 
    n_z = length(z)
    n_N = length(y_idx)
    n_y = length(y)
    p_X = ncol(X)
    p_H = ncol(H)
    n_zeta = ncol(watershed_mat)
    T = ncol(year_mat)
    n = ncol(site_mat)
    n_q = length(D)
    
    ### Priors 
    Sigma_beta_inv = solve(Sigma_beta)
    Sigma_beta_inv_times_mu = Sigma_beta_inv %*% mu_beta
    Sigma_gamma_inv = solve(Sigma_gamma)
    Sigma_gamma_inv_times_mu = Sigma_gamma_inv %*% mu_gamma
    ### How we effectively set the coefficient on area (S) to 1 
    coef_precision = 1e5
    
    ### Objects 
    kappa = z - rep(1, n_z) / 2
    I = diag(1, n_z)
    zeta_COV_inv = (diag(rowSums(A)) - 0.999 * A)
    zeta_COV = solve(zeta_COV_inv)
    q_zeta = n_zeta / 2 + var_shape
    q_xi = T / 2 + var_shape
    q_eta = map_dbl(D, ncol) / 2 + var_shape
    M = cbind(log(S), X, watershed_mat, year_mat, site_mat)
    M_z = M[1:n_z,]
    M_N = M[-(1:n_z),]
    H_big = H[rep(seq_len(nrow(H)), times = J_y), ]
    
    ###
    ### Starting Values
    ###
    
    beta = mu_beta
    gamma = mu_gamma
    pi_logit = H %*% gamma
    pi = logit_inv(pi_logit)
    zeta = numeric(n_zeta)
    xi = numeric(T)
    eta = numeric(n)
    log_lambda = numeric(n_z+n_N)
    N = y_sum
    nu = 1
    sigma2_zeta = 1
    sigma2_xi = 1
    sigma2_eta = 1
    phi = rep(100, n_q)
    eta_COV = map2(D, phi, ~exp(-.x/.y))
    eta_COV_chol = chol(bdiag(eta_COV))
    eta_COV_inv = chol2inv(eta_COV_chol)
    rho = 0.5
    xi_COV = make_ar1_corr(T, rho)
    xi_COV_chol = chol(xi_COV)
    xi_COV_inv = chol2inv(xi_COV_chol)
    mean_vec = c(coef_precision, Sigma_beta_inv_times_mu, rep(0, n_zeta + T + n))
    coef_inv = diag(coef_precision, 1)
    
    ### Get eta index
    sizes = vapply(eta_COV, ncol, integer(1))  # block sizes
    starts = c(1, cumsum(sizes[-length(sizes)]) + 1)
    ends = cumsum(sizes)
    # Return list of index vectors
    eta_idx = Map(seq, starts, ends)
    
    ###
    ### Acceptance
    ###
    
    rho_acc = 0
    phi_acc = numeric(n_q)
    N_acc = numeric(n_N)
    rho_tune = 2.4^2 
    phi_tune = rep(2.4^2, n_q)
    ### Hyperparameters for tuning adaptive sampler 
    r_opt = 0.234
    
    ###
    ### Storage
    ###
    
    beta_save = matrix(NA, p_X, n_mcmc)
    gamma_save = matrix(NA, p_H, n_mcmc)
    N_save = matrix(NA, n_N, n_mcmc)
    zeta_save = matrix(NA, n_zeta, n_mcmc)
    xi_save = matrix(NA, T, n_mcmc)
    eta_save = matrix(NA, n, n_mcmc)
    log_lambda_save = matrix(NA, n_z+n_N, n_mcmc)
    sigma2_zeta_save = numeric(n_mcmc)
    sigma2_eta_save = matrix(NA, n_q, n_mcmc)
    sigma2_xi_save = numeric(n_mcmc)
    rho_save = numeric(n_mcmc)
    phi_save = matrix(NA, n_q, n_mcmc)
    coef_save = numeric(n_mcmc)
    
    ###
    ### MCMC loop
    ###
    
    for (q in 1:n_mcmc) {
      
      ### For automatic tuning 
      tune = 1.0 / q^c_1
      
      if(count){
        
        ###
        ### Sample N
        ###
        
        for(i in 1:n_N){
          
          # Proposal value
          N_star = rpois(1, N[i])
          
          if(N_star >= y_sum[i]){
            
            # Metropolis-Hastings ratio
            mh1 = NB_polson_N(N_star, nu, log_lambda[i+n_z]) +
                        dpois(N[i], N_star, TRUE)
            mh2 = NB_polson_N(N[i], nu, log_lambda[i+n_z]) +
                        dpois(N_star, N[i], TRUE)
            
            for(j in 1:J_y[i]){
              mh1 = dbinom(y[y_idx[i]+j], N_star-N_deplete[y_idx[i]+j], pi[i], TRUE) + mh1
              mh2 = dbinom(y[y_idx[i]+j], N[i]-N_deplete[y_idx[i]+j], pi[i], TRUE) + mh2
            }
            
            mh = exp(mh1 - mh2)
            
            # Accept or reject
            if (runif(1) < mh) {
              N[i] = N_star
              N_acc[i] = N_acc[i] + 1
            }
            
          }
          
        }
        N_big = rep(N, J_y) - N_deplete
        non_zero_idx = which(N_big!=0)
        
        ###
        ### Sample gamma
        ###
        
        ### Sample omega
        omega = rpg(length(non_zero_idx), N_big[non_zero_idx], rep(pi_logit, J_y)[non_zero_idx])
        
        ### Sample gamma
        tmp_chol = chol(t(H_big[non_zero_idx,]) %*% (omega * H_big[non_zero_idx,]) + Sigma_gamma_inv)
        gamma = backsolve(tmp_chol, 
                          backsolve(tmp_chol,  
                                    t(H_big[non_zero_idx,]) %*% (y[non_zero_idx]-N_big[non_zero_idx]/2),
                                    transpose=TRUE) + rnorm(p_H)) 
        pi_logit = H %*% gamma
        pi = logit_inv(pi_logit)
        
      }
      
      ###
      ### Sample fixed and random effects 
      ###
      
      ### Sample omegas
      if(binary){
        omega_z = rpg(n_z, 1, log_lambda[1:n_z])
      }
      if(count){
        omega_N = rpg(n_N, N+nu, log_lambda[-(1:n_z)])
      }
      
      ### Sample beta, zeta, xi, and eta
      var_inv = as.matrix(bdiag(coef_inv, 
                                Sigma_beta_inv, 
                                (1 / sigma2_zeta) * zeta_COV_inv, 
                                (1 / sigma2_xi) * xi_COV_inv, 
                                eta_COV_inv))
      tmp_var_inv = var_inv
      tmp_mean = mean_vec
      if(binary){
        tmp_var_inv = tmp_var_inv + t(M_z) %*% (omega_z * M_z)
        tmp_mean = tmp_mean + t(M_z) %*% kappa
      }
      if(count){
        tmp_var_inv = tmp_var_inv + t(M_N) %*% (omega_N * M_N)
        tmp_mean = tmp_mean + t(M_N) %*% (N-nu)/2 
      }
      tmp_chol = chol(tmp_var_inv)
      coef = backsolve(tmp_chol, 
                       backsolve(tmp_chol,  
                                 tmp_mean,
                                 transpose=TRUE) + rnorm(p_X + n_zeta + T + n + 1)) 
      log_lambda = M %*% coef
      coef_save[q] = coef[1]
      coef = coef[-1]
      beta = coef[1:p_X]
      zeta = coef[(p_X + 1):(p_X + n_zeta)]
      xi = coef[(p_X + n_zeta + 1):(p_X + n_zeta + T)]
      eta = coef[(p_X + n_zeta + T + 1):(p_X + n_zeta + T + n)]
      
      ### Sample sigma2_zeta
      r_tilde = t(zeta) %*% zeta_COV_inv %*% zeta / 2 + var_rate
      sigma2_zeta = 1 / rgamma(1, shape = q_zeta, scale = 1 / r_tilde)
      
      ### Sample sigma2_xi
      r_tilde = t(xi) %*% xi_COV_inv %*% xi / 2 + var_rate
      sigma2_xi = 1 / rgamma(1, shape = q_xi, scale = 1 / r_tilde)
      
      ### Sample sigma2_eta
      r_tilde = map2_dbl(eta_idx, sigma2_eta, ~sum(t(eta[.x]) %*% (.y * eta_COV_inv[.x,.x]) %*% eta[.x] / 2 + var_rate)) 
      sigma2_eta = 1 / rgamma(n_q, shape = q_eta, scale = 1 / r_tilde)
      
      ###
      ### Sample rho
      ###
      
      # random walk proposals
      rho_star = logit_inv(rnorm(1, logit(rho), sd=sqrt(rho_tune)))
      
      if(0.01 < rho_star & rho_star < 0.99){
        
        # Calculate AR(1) correlation matrix 
        xi_COV_star = sigma2_xi * make_ar1_corr(T, rho_star)
        xi_COV_chol_star = chol(xi_COV_star)
        xi_COV_chol = chol(sigma2_xi * make_ar1_corr(T, rho))
        
        # Under proposal 
        mh1 =
          # Likelihood
          (-0.5) * sum(backsolve(xi_COV_chol_star, xi, transpose = TRUE)^2) + 
          (-0.5) * 2 * sum(log(diag(xi_COV_chol_star))) 
        
        # Under current values
        mh2 =
          # Likelihood
          (-0.5) * sum(backsolve(xi_COV_chol, xi, transpose = TRUE)^2) + 
          (-0.5) * 2 * sum(log(diag(xi_COV_chol))) 
        
        # MH ratio
        mh = exp(mh1-mh2)
        
        # Accept or reject
        if (runif(1) < mh) {
          rho = rho_star
          xi_COV = xi_COV_star
          xi_COV_inv = chol2inv(xi_COV_chol_star)
          rho_acc = rho_acc + 1
          rho_tune = exp(log(rho_tune) + tune*(1.0 - r_opt));
        } else { 
          rho_tune = exp(log(rho_tune) + tune*(0.0 - r_opt));
        } 
        
      } else {
        rho_tune = exp(log(rho_tune) + tune*(0.0 - r_opt));
      }
      
      ###
      ### Sample phi
      ###
      
      for (i in 1:n_q) {
        
        # Random walk proposal for network i
        phi_star = exp(rnorm(1, log(phi[i]), sd = sqrt(phi_tune[i])))
        
        # Calculate correlation matrix for this network
        eta_COV_star_i = exp(-D[[i]] / phi_star)
        
        # Attempt Cholesky
        chol_success = TRUE
        eta_COV_chol_star_i = tryCatch({
          suppressWarnings(chol(sigma2_eta[i] * eta_COV_star_i))
        }, error = function(e) {
          chol_success <<- FALSE 
          return(NULL) 
        })
        
        # Only compute MH ratio if Cholesky succeeded
        if (chol_success & phi_star > 10) {
          
          # Current correlation matrix Cholesky
          eta_COV_chol_i = chol(sigma2_eta[i] * eta_COV[[i]])
          eta_tmp = eta[eta_idx[[i]]]
          
          # Likelihood under proposal
          mh1 = 
            (-0.5) * sum(backsolve(eta_COV_chol_star_i, eta_tmp, transpose = TRUE)^2) +
            (-0.5) * 2 * sum(log(diag(eta_COV_chol_star_i))) +
            # Prior on phi 
            dgamma(phi_star, shape=phi_shape, rate=phi_rate, log=TRUE)
          
          # Likelihood under current
          mh2 = 
            (-0.5) * sum(backsolve(eta_COV_chol_i, eta_tmp, transpose = TRUE)^2) +
            (-0.5) * 2 * sum(log(diag(eta_COV_chol_i))) +
            # Prior on phi 
            dgamma(phi[i], shape=phi_shape, rate=phi_rate, log=TRUE)
          
          # MH ratio
          mh = exp(mh1 - mh2)
          
          # Accept or reject
          if (runif(1) < mh) {
            phi[i] = phi_star
            eta_COV[[i]] = eta_COV_star_i
            phi_acc[i] = phi_acc[i] + 1
            phi_tune[i] = exp(log(phi_tune[i]) + tune*(1.0 - r_opt))
          } else {
            phi_tune[i] = exp(log(phi_tune[i]) + tune*(0.0 - r_opt))
          }
          
        } else {
          # Skip update if Cholesky failed
          phi_tune[i] = exp(log(phi_tune[i]) + tune*(0.0 - r_opt))
        }
      }
      
      ### Update eta_COV_inv 
      eta_COV_inv = bdiag(map2(eta_COV, sigma2_eta, ~solve(.y * .x)))
      
      ### Save Samples
      log_lambda_save[, q] = as.vector(log_lambda)
      N_save[, q] = N
      gamma_save[, q] = gamma
      beta_save[, q] = beta
      zeta_save[, q] = zeta
      xi_save[, q] = xi
      eta_save[, q] = eta
      sigma2_zeta_save[q] = sigma2_zeta
      sigma2_eta_save[, q] = sigma2_eta
      sigma2_xi_save[q] = sigma2_xi
      rho_save[q] = rho
      phi_save[, q] = phi
      
      ### Timer
      if (q %% 1000 == 0) cat(q, " ")
    }
    
    
    list(
      ## MCMC samples
      coef=coef_save,
      log_lambda=log_lambda_save,
      N=N_save,
      gamma=gamma_save,
      beta=beta_save,
      zeta=zeta_save,
      xi=xi_save,
      eta=eta_save,
      sigma2_zeta=sigma2_zeta_save,
      sigma2_eta=sigma2_eta_save,
      sigma2_xi=sigma2_xi_save,
      rho=rho_save,
      phi=phi_save,
      ## Acceptance 
      rho_tune=rho_tune,
      rho_acc=rho_acc,
      phi_tune=phi_tune,
      phi_acc=phi_acc,
      N_acc=N_acc
    )
  }
